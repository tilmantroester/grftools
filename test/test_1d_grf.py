...
def test_gaussian_random_field_generator(plot=False):
    ...
    d = grftools.onedee.random_fields.create_gaussian_random_field(P=P, n_grid=n_grid, L=L)
    P_data, k_data = grftools.onedee.random_fields.pseudo_Pofk(d, d, L)
    P_data = P_data.real
...
def test_multiple_gaussian_random_field_generator(plot=False):
    ...
    d1, d2 = grftools.onedee.random_fields.create_gaussian_random_fields(power_spectra=[P["AA"], P["BB"], P["AB"]], 
                                                                       n_grid=n_grid, L=L)
    P_data = {}
    P_data["AA"], k_data = grftools.onedee.random_fields.pseudo_Pofk(d1, d1, L)
    P_data["BB"], k_data = grftools.onedee.random_fields.pseudo_Pofk(d2, d2, L)
    P_data["AB"], k_data = grftools.onedee.random_fields.pseudo_Pofk(d1, d2, L)
    ...
    for key in ["AA", "BB", "AB"]:
        ...
        if key[0] == key[1]:
            ...
            tolerace_p_value = 0.03
            cdf = lambda x: grftools.onedee.random_fields.variance_gamma_distribution_cdf(x, n=2, sigma1=1, sigma2=1, rho=rho)
    ...
    if plot:
        ...
        for i, key in enumerate(["AA", "BB", "AB"]):
            if key[0] == key[1]:
                ...
                q = grftools.onedee.random_fields.variance_gamma_distribution_ppf([0.16, 0.84], n=2, rho=rho)
        ...
        for i, key in enumerate(["AA", "BB", "AB"]):
            if key[0] == key[1]:
                ...
                x = np.linspace(b[1][0], b[1][-1], 100)
                distr = grftools.onedee.random_fields.variance_gamma_distribution(x, n=2, rho=rho)
